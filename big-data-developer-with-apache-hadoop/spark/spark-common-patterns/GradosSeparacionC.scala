// $ mvn package
// $ spark-submit --class example.GradosSeparacionC --name 'Grados de Separacion' --master yarn-client target/grados-separacion-1.0.jar /files/Marvel-graph.txt
// $ spark-submit --class example.GradosSeparacionC --name 'Grados de Separacion' --master 'local[*]' target/grados-separacion-1.0.jar /files/Marvel-graph.txt
// $ spark-submit --class example.GradosSeparacionC --name 'Grados de Separacion' --master 'local[8]' target/grados-separacion-1.0.jar /files/Marvel-graph.txt
 ...
 * funci�n noHayGrises. Tenemos un acumulador, numGrises, que incializamos a
 * cero. Luego cuando la funci�n bfsMap crea un nuevo nodo de color gris incrementa el acumulador.
 * Despu�s de hacer el map/reduce en el main comprobamos el valor del acumulador. Si
 * es mayor que 0, lo ponemos a 0 y seguimos en el bucle. Si es 0 el bucle ha terminado.
 * En el main, despu�s de hacer el reduce, ejecutamos take(1) para provcar una
 * acci�n. Si no lo hacemos, no se ejecuta el map ni el reduce ya que Spark trabaja en
 * modo lazy. Y entonces el programa se termina.
 *
 * grados-de-separacion/grados-de-separacionC.scala
 * Como grados-de-separaci�n.scala pero para calcular el final comprobramos si ya lo ha
 * encontrado o si no hay m�s grises. Ambas cosas las hacemos con un acumulador.
 *
 ...
 */
object GradosSeparacionC {

  val B = "BLANCO"
  val G = "GRIS"
  val N = "NEGRO"

def setInit(line: (String, (Array[String], Int, String)), encontrado: Accumulator[Int] ): (String, (Array[String], Int, String)) = {
	if(line._1.equals("1")) {
		encontrado.add(line._2._1.length)		
		(line._1,(line._2._1,0,G))
	} else line
}

def changeToBlack(value: (Array[String], Int, String), encontrado: Accumulator[Int] ): (Array[String], Int, String) = {
	if(value._3.equals(G)){
		if(value._1.length == encontrado.localValue) encontrado.add(-encontrado.localValue)
  		(value._1,value._2 ,N)
	} else value	
}

def mixValues(v1: (Array[String], Int, String) ,v2: (Array[String], Int, String), encontrado: Accumulator[Int] ): (Array[String], Int, String)  = {
	if(v1._3.equals(N)) v1
	else if (v2._3.equals(N)) v2
	else if(v1._1(0).equals("")) {
		val distancia = v1._2
		val color = v1._3
		if(v2._1.length > encontrado.localValue) {
			encontrado.add(-encontrado.localValue)
			encontrado.add(v2._1.length)
		}
		(v2._1, distancia, color)
	} else {
		val distancia = v2._2
		val color = v2._3
		if(v1._1.length > encontrado.localValue) {
			encontrado.add(-encontrado.localValue)
			encontrado.add(v1._1.length)
		}
   ...
   * y construye un mapa que devuelve en la salida
   * Usa tailrecursion para iterar
   */
  ...
  */
  ...
  def iterar(acumulador: Accumulator[Int],encontrado: Accumulator[Int], heroesM: org.apache.spark.rdd.RDD[(String, (Array[String], Int, String))]):org.apache.spark.rdd.RDD[(String, (Array[String], Int, String))] = {
			...
			println("La distancia es %d".format(encontrado.localValue))

			if(acumulador.localValue > 0 && encontrado.localValue > 0) {
				val heroesM1 = heroesM.mapValues(l => changeToBlack(l, encontrado) )
				...
				val heroesM3 = heroesM2.reduceByKey{case ((hs,d,c),(hs1,d1,c1))  => mixValues((hs,d,c),(hs1,d1,c1), encontrado)}

				heroesM2.unpersist()

				iterar(acumulador, encontrado, heroesM3.persist())
			} else {
				heroesM
			}
  ...
  def main(args: Array[String]) {
    ...
    val encontrado = sc.accumulator(0)
    ...
    val heroes = iterar(acumulador, encontrado, sc.textFile(myFile).map(l => (l.split(' ')(0), (l.split(' ').tail, infinity, B))).map(l => setInit(l, encontrado)))
