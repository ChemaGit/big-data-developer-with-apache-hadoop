/**
  * En esta versi�n hay que sacar la temperatura media por estaci�n y por a�o. La
...
/**
 * Calcular la media de las temperaturas m�nimas por estaci�n y a�o
 */
 ...
 /**
  * Calcular la media de las temperaturas m�ximas por estaci�n y a�o
  */
  val dirTemp = "/files/1800_1801.csv"
  val rddTemp = sc.textFile(dirTemp).map(line => line.split(",")).map({case(arr) => ( (arr(0), arr(1).substring(0,4)), arr(2),arr(3).toDouble )})
