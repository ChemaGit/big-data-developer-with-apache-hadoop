 ...
 */
XXX = --master "local[8]"
$ spark-submit --class com.hadoopexam.MyTask --master "local[8]" --deploy-mode cluster /SPARK_HOME/lib/hadoopexam.jar 10
...
To submit with --deploy-mode cluster, the HOST:PORT should be configured to connect to the MesosClusterDispatcher. yarn Connect to a YARN cluster in client or cluster mode depending on the value of - deploy-mode. 
